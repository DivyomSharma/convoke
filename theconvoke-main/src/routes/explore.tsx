...
export const Route = createFileRoute("/explore")({
  head: () => ({
    meta: [
      { title: "Explore — Convoke" },
      { name: "description", content: "One ecosystem feed: events, jobs, hackathons, communities, projects, founders, drops." },
    ],
  }),
  component: Explore,
});
          ...
          {items.map((it) => (
            <li key={it.id} className="py-7">
              <div className="flex items-center gap-3 mb-3">
                <Link to="/profile/$handle" params={{ handle: it.who.handle }}>
                  <Avatar src={it.who.avatar} name={it.who.name} size={28} />
                </Link>
                <Link to="/profile/$handle" params={{ handle: it.who.handle }} className="text-[14px] text-ink hover:underline">
                  {it.who.name}
                </Link>
