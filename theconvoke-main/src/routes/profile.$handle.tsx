...
export const Route = createFileRoute("/profile/$handle")({
  loader: ({ params }) => {
    const p = people.find((x) => x.handle === params.handle);
    if (!p) throw notFound();
    return p;
  },
  head: ({ loaderData }) => ({
    meta: [
      { title: `${loaderData?.name ?? "Profile"} — Convoke` },
      { name: "description", content: `${loaderData?.name ?? ""} on Convoke — ${loaderData?.role ?? ""}.` },
      { property: "og:title", content: `${loaderData?.name ?? "Profile"} — Convoke` },
      { property: "og:image", content: loaderData?.avatar },
    ],
  }),
  notFoundComponent: () => (
    <Shell><div className="mx-auto max-w-xl px-5 py-24 text-center"><div className="eyebrow">No one here</div><h1 className="serif text-5xl mt-3">That profile doesn't exist.</h1></div></Shell>
  ),
  component: Profile,
});
