...
const mockChallenges = [
  { id: 1, title: "Global AI Challenge", status: "Live", participants: 1240, prize: "$50k", date: "Oct 20 - Nov 10", location: "Online", org: "OpenAI Labs" },
  ...
  { id: 3, title: "Design Sprint 2026", status: "Registration Open", participants: 800, prize: "₹5 Lakhs", date: "Jan 15 - Jan 17", location: "New Delhi", org: "Figma" },
];
          ...
          {mockChallenges.map((challenge) => (
            <Link key={challenge.id} href={`/challenges/${challenge.id}`} className="premium-card overflow-hidden group flex flex-col">
              ...
              <div className="p-6 flex-1 flex flex-col">
                ...
                <p className="text-g5 text-[14px] mt-1">by {challenge.org}</p>
                
                <div className="mt-6 flex-1 flex flex-col justify-end gap-3 text-[13px] text-g6">
                  <div className="flex items-center gap-2"><Trophy size={14} className="text-g4" /> <span className="font-medium text-ink">{challenge.prize}</span> in prizes</div>
                  <div className="flex items-center gap-2"><Users size={14} className="text-g4" /> {challenge.participants} builders going</div>
                  <div className="flex items-center gap-2"><MapPin size={14} className="text-g4" /> {challenge.location} · {challenge.date}</div>
                </div>
              </div>
            </Link>
          ))}
