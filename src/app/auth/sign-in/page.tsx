...
export default function SignInPage() {
  ...
  const [status, setStatus] = useState<"idle" | "loading" | "signup">("idle");
  ...
  const emailPassword = async (mode: "login" | "signup") => {
    ...
    setStatus(mode === "login" ? "loading" : "signup");
    ...
    try {
      ...
      setStatus("idle");
      setMessage(readAuthError(error, "Auth failed. Check your credentials or try again."));
    }
  };
  ...
  const oauthLogin = async (provider: "github" | "google") => {
    ...
    try {
      ...
      setMessage(readAuthError(error, `Could not start ${provider} login.`));
    }
  };
      ...
      <section className="flex items-center justify-center px-5 py-10">
        <Panel className="w-full max-w-md p-6">
          ...
          <div className="mt-7 grid gap-3">
            <Button variant="secondary" onClick={() => oauthLogin("google")} className="justify-start">
            ...
            <Button variant="secondary" onClick={() => oauthLogin("github")} className="justify-start">
          ...
          {message && (
            <div className={`mt-4 flex gap-2 rounded-[8px] border p-3 text-sm ${
          ...
          <Button className="mt-4 w-full" disabled={status !== "idle"} onClick={() => emailPassword("login")}>
            {status === "loading" && <Loader2 className="size-4 animate-spin" />}
            Sign in with email
          </Button>
          <Button className="mt-3 w-full" disabled={status !== "idle"} onClick={() => emailPassword("signup")} variant="secondary">
            {status === "signup" && <Loader2 className="size-4 animate-spin" />}
